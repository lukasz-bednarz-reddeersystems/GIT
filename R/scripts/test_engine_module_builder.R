...
client_emulation <- writeToConnection(client_emulation,paste("SETDATE|2016-07-01",sep=""))
client_emulation <- readConnection(client_emulation)
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
...
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
client_emulation <- writeToConnection(client_emulation,"SETMODULE|PortfolioFactorExposureAnalysisBlock")
client_emulation <- readConnection(client_emulation)
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
...
#Now should be able to access the shiny app in a browser
client_emulation <- readConnection(client_emulation)
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
