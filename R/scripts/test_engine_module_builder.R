...
client_emulation <- writeToConnection(client_emulation,paste("SETDATE|2016-01-01",sep=""))
client_emulation <- readConnection(client_emulation)
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
...
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
client_emulation <- writeToConnection(client_emulation,"SETMODULE|StrategyBreakdownAnalysisBlock")
client_emulation <- readConnection(client_emulation)
resp <- getDataFromSocket(client_emulation)
message(paste("Got:",resp))
