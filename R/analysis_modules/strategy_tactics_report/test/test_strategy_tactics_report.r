sourceTo("../analysis_modules/strategy_tactics_report/strategy_tactics_report.r", modifiedOnly = getOption("modifiedOnlySource"), local = FALSE)
...
test_that(paste("Can Process() on", tested.class), {
  ...
  expect_true(setequal(names(getOutputGGPlotList(object)), 
                       c(strategy_tactics_report_analysis_blocks, "Summary")))
  
  expect_true(setequal(names(getOutputGGPlotDataList(object)), 
                       strategy_tactics_report_analysis_blocks))
  
  expect_true(setequal(names(getOutputObjectList(object)), 
                       setdiff(strategy_tactics_report_analysis_blocks, 
