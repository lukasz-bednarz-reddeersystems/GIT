sourceTo("../analysis_modules/trading_review_report/trading_review_2016.2_report.r", modifiedOnly = getOption("modifiedOnlySource"), local = FALSE)
sourceTo("../models/key_library.r", modifiedOnly = getOption("modifiedOnlySource"), local = FALSE)
library(testthat)

#########################
#
# TradingReview2016.2Report Tests
#
#########################
tested.class             <-  "TradingReview2016.2Report"
...
test_that(paste("Can Process() on", tested.class), {
  ...
  expect_true(setequal(names(getOutputGGPlotList(object)), 
                       c(trading_review_2016.2_report_analysis_blocks, "Summary")))
  
  expect_true(setequal(names(getOutputGGPlotDataList(object)), 
                       trading_review_2016.2_report_analysis_blocks))
  
  expect_true(setequal(names(getOutputObjectList(object)), 
                       setdiff(trading_review_2016.2_report_analysis_blocks, 
                               c("AverageDownTradesFocus", "PositionsHoldingDayZeroPnL",
                                 "PositionsHoldingCapitalDistribution", "StrategyBreakdownAUMAndTurnover",
                                 "StrategyBreakdownTotalAndDeltaPnL", "StrategyBreakdownPnLAndTurnoverPerEvent",
                                 "PortfolioFactorExposure", "PositionRevisitsDeltaPrevQuarter"  ))))
