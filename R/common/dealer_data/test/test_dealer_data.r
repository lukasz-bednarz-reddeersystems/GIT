sourceTo("../common/dealer_data/dealer_data.r", modifiedOnly = getOption("modifiedOnlySource"), local = FALSE)
library(testthat)
library(plyr)
...
test_that("Can dataRequest() with valid key_values", {
  ...
  valid.fact_transf <- factor_transform(valid.ret_data[c(valid.factor_keys,valid.factor_cols)], valid.factor_keys, valid.factor_cols)
