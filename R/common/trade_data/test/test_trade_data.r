...
#
#########################
tested.class          <-  "TradeData"
valid.key_cols        <- c("id", "start", "end")
valid.values          <- c(trade_basic_features,
                           outcome_price_features, 
                           context_price_features, 
                           position_summary_features,  
                           trade_history_features,
                           trade_delta_features)
valid.required_colnms <- c(trade_basic_features_transl,
                           outcome_price_features, 
                           context_price_features, 
                           position_summary_features,  
                           trade_history_features ,
                           trade_delta_features)
valid.column_name_map <- hash(c(trade_basic_features, 
                                outcome_price_features, 
                                context_price_features, 
                                position_summary_features,  
                                trade_history_features ,
...
valid.model_class     <- "TradeHistorySimpleWithSummary"
...
test_that("Can dataRequest() with valid key_values", {
  ...
  colnames(valid.ret_data) <- .translateDataSourceColumnNames(object, valid.values)
  
  # valid.ret_data <- unique(valid.ret_data)
  # valid.ret_data <- valid.ret_data[!is.na(valid.ret_data$TradeID),]
