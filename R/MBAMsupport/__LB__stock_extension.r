...
ind_x <- !is.na(x)
x <- x[ind_x]
x[lofactor(x, k = 5) > lf_thr] <- NA
trades$NFromMAVG20[ind_x] <- scale(x)
...
x <- x[ind_x]
x[lofactor(x, k = 5) > lf_thr] <- NA
trades$NFromMAVG50[ind_x] <- scale(x)
...
x <- x[ind_x]
x[lofactor(x, k = 5) > lf_thr] <- NA
trades$RSI14[ind_x] <- scale(x)

x <- trades$RelativeRSI14
x[is.infinite(x) | is.nan(x)] <- NA
ind_x <- !is.na(x)
x <- x[ind_x]
x[lofactor(x, k = 5) > lf_thr] <- NA
trades$RelativeRSI14[ind_x] <- scale(x)
