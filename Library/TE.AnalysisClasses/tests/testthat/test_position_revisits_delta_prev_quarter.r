context("Testing PositionRevisitsDeltaPrevQuarterAnalysisBlock")

######################################################
#
# PositionRevisitsDeltaPrevQuarterAnalysisBlock Tests
#
######################################################


tested.class          <-  "PositionRevisitsDeltaPrevQuarterAnalysisBlock"
...
test_that("Can dataRequest() with valid key_values", {
  ...
  valid.key_values <- dated_three_monthly_lookback(11, today())
...
test_that(paste("Can Process() on", tested.class, "after requesting Data"), {
  skip_if_not(as.logical(Sys.getenv("R_TESTTHAT_RUN_LONG_TESTS", unset = "FALSE")))

  object <- new(tested.class)

  valid.key_values <- dated_twelve_monthly_lookback(11, as.Date("2016-07-30"))
  colnames(valid.key_values) <- c("TraderID", "start", "end")
