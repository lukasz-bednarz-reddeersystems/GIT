...
sourceTo("../common/datastore_client/test/test_datastore_client_class.r", modifiedOnly = getOption("modifiedOnlySource"), local = FALSE)
library(testthat)
library(plyr)
library(lubridate)
...
test_that("Can dataRequest() with valid key_values", {
  ...
  tr_data <- factor_transform(res_data[c(key_names, factor_cols)], key_names, factor_cols )
  res_data <- cbind(res_data[setdiff(colnames(res_data), factor_cols)],
                    tr_data[setdiff(colnames(tr_data), colnames(res_data))])
  
  colnames(res_data)[1:4] <- LETTERS[1:4]
